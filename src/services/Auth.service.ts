...
import getApiAccessToken, { changeTemporaryPassword, resetTemporaryPassword } from '../utils/AwsCognito.util';
...
export async function getUserDetails() {
  ...
  const res = await axios.get(`${baseURL}/${version}/user`, optionForUserDetail);
  return res.data;
}

export async function updateUserDetails(requestBody: any) {
  const res = await axios.post(`${baseURL}/${version}/user/updateUser`, requestBody, optionForProductDetails);
  return res.data;
}

export function acceptEula(requestBody: any) {
  if (serveFromMockoon) {
    return BaseUrl.post('/api/v3/eula-accept', options).then((res: any) => res.data);
  }
  const option = {
    headers: {
      'Content-Type': 'application/json',
  ...
  return axios.post(`${baseURL}/${version}/eula-accept`, requestBody, option).then((res: any) => res.data);
}
...
export function classLinkDetails(requestBody: any) {
  if (serveFromMockoon) {
    return BaseUrl.post('/api/v3/user/roster-authentication', options).then((res: any) => res.data);
  }
  return axios.post(`${baseURL}/${version}/user/roster-authentication`, requestBody, optionForProductDetails).then((res: any) => {
    return res.data;
  });
}
...
export function getPrivacyPolicy() {
  return BaseUrl.get('product/privacyPolicy', options).then((res: any) => res.data);
}

export function getGameDetails() {
  return BaseUrl.get('product/gameDetails', options).then((res: any) => res.data);
}
